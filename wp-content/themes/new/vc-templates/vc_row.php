...
if(!empty($anchor)){
	$wrapper_attributes[] = 'data-rh-anchor="' . esc_attr($anchor ) . '"';
}
...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(rh_options()->getOptionValue('parallax_on_off') == 'off'){
		$css_classes[] = 'rh-parallax-section-holder-touch-disabled';
	}
	if($parallax_speed != ''){
...
if($video == 'show_video'){
	...
	$video_overlay_class = 'rh-video-overlay';
	$video_overlay_style = '';
	$video_mobile_style = '';
	$video_attrs = '';
	$v_image = '';
	if($video_overlay == "show_video_overlay"){
		$video_overlay_class .= ' rh-video-overlay-active';
	}
	if($video_image) {
	...
	if($video_image) {
		$video_output .= '<div class="rh-mobile-video-image" ' . rh_get_inline_attr($video_mobile_style, 'style') . '></div>';
	}
	$video_output .= '<div ' . rh_get_class_attribute($video_overlay_class) . rh_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="rh-video-wrap">';
	$video_output .= '<video class="rh-video" width="1920" height="800" '. rh_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . rh_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="'.esc_attr_e( 'No video playback capabilities', 'xpo' ).'" alt="'.esc_attr__('video thumb','xpo').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
